...
/**
  ...
  * @retval ESP_FAIL if fail, else ESP_OK
  */
esp_err_t get_self_ip_station(uint32_t *self_ip)
{
	if (ESP_OK != get_ipaddr_from_str(INPUT_STATION_SRC_IP, self_ip)) {
...
/**
  ...
  * @retval ESP_FAIL if fail, else ESP_OK
  */
esp_err_t get_self_ip_softap(uint32_t *self_ip)
{
	if (ESP_OK != get_ipaddr_from_str(INPUT_SOFTAP_SRC_IP, self_ip)) {
...
/**
  ...
  * @retval ESP_FAIL if fail, else ESP_OK
  */
esp_err_t get_arp_dst_ip_station(uint32_t *sta_ip)
{
	if (ESP_OK != get_ipaddr_from_str(INPUT_STATION_ARP_DEST_IP, sta_ip)) {
...
/**
  ...
  * @retval ESP_FAIL if fail, else ESP_OK
  */
esp_err_t get_arp_dst_ip_softap(uint32_t *soft_ip)
{
	if (ESP_OK != get_ipaddr_from_str(INPUT_SOFTAP_ARP_DEST_IP, soft_ip)) {
...
void control_path_init(void(*control_path_evt_handler)(uint8_t))
{
	...
	// xTaskCreate(control_path_task, "control_path_task", CONTROL_PATH_TASK_STACK_SIZE, NULL, 3, NULL);
}
...
/**
  ...
  * @retval ESP_OK/ESP_FAIL
  */
static esp_err_t save_softap_mac(const char *mac)
{
...
/**
  ...
  * @retval ESP_OK/ESP_FAIL
  */
static esp_err_t save_station_mac(const char *mac)
{
...
int station_connect(void)
{
	...
	control_path_call_event(STATION_STARTED);
